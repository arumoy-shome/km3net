...
*** Experiments with optimizers
    ...
    Experiments were first carried out with the 10% dataset variant
    (since training is quicker) and the /lr/ of the best model was
    applied to the 50% dataset variant to obtain the final model of
    the category. See =notebooks/pm/exp-optim.ipynb= for more details.
